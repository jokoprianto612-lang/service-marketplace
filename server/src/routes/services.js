...
const deploySchema = zod_1.z.object({
    definitionId: zod_1.z.string(),
    name: zod_1.z.string().min(1).max(100),
    mode: zod_1.z.enum(['quick', 'custom', 'stack']).default('quick'),
    config: zod_1.z.record(zod_1.z.any()).optional(),
    targetId: zod_1.z.string().optional(),
});
const scaleSchema = zod_1.z.object({
    replicas: zod_1.z.number().min(0).max(10),
});
...
async function serviceRoutes(app) {
    ...
    app.post('/', {
        ...
        try {
            await (0, job_queue_1.queueDeployment)(app, {
                instanceId: instance.id,
                definitionId: body.definitionId,
                projectId: user.projectId,
                userId: user.id,
